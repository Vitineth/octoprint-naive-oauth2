// Adapted from src/octoprint/static/js/app/viewmodels/usersettings.js
//   With modifications to add support for the OAuth user modifications

$(function () {
    function UserSettingsWithOAuthViewModel(parameters) {
        var self = this;
        ...
        console.log(self.loginState.currentUser());
        console.log(self.loginState.currentUser.subscribe(() => console.log("HELLO")));
        ...
        self.currentUser = ko.observable(self.loginState.currentUser());
        ...
        self.loginState.currentUser.subscribe((user) => {
        ...
        self.isOAuthUser = ko.pureComputed(function () {
            console.log('oauth called', self.currentUser(), self.getSetting('oauth.is_oauth_user') === 'true');
            return self.getSetting('oauth.is_oauth_user') === 'true';
        });

        self.getSetting = function (setting, fallback) {
            const u = self.currentUser();
            if (u && u.settings && u.settings[setting]) {
                return u.settings[setting];
            }
    ...
    OCTOPRINT_VIEWMODELS.push({
        construct: UserSettingsWithOAuthViewModel,
        dependencies: ["loginStateViewModel", "accessViewModel"],
        elements: ["#usersettings_plugin_naive_oauth2"]
    });
});
